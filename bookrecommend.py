...
from database import searchData, getDate,getData
...
def bookrecommend (memberID):
    ...
    bookCount = 10
    if memberBooks:
        #checks if the member has any previous books
        ...
        #if the member hasn't read any previous books the program
        #appends the 10 most read books in the system
        f= open("database.txt","r")
        books = []
        for allBooks in f:
                books.append(getData(allBooks.strip("\n")))
        f.close()
        del books[0]
        allBooks = {}
        for eachBook in books:
        ...
        allBooks = {k: v for k, v in sorted(allBooks.items(),
                                            key=lambda item: item[1])}
        allBooks = list(allBooks.items())[:10]
        for books in allBooks:
            bookDict[books[0]] = books[1]

    while len(bookDict) > 10:
        #removes least popular books if it is more then 10

        del bookDict[list(bookDict.keys())[0]]
...
#--------------------------------------Testing---------------------------------
def testing_bookrecommend_noBooks():
    #test for when a user has not read any books
    actual = bookrecommend("abcd")
    expected = ['Lethal White', 'Cold Waters', 'Eligible', 'The Road',
                "Angela's Ashes", 'The Last of the Mohicans', 'Scoop',
                'The Whistler', 'The Fifth Season', 'The Shipping News']
    assert  actual[1] == expected, "should show a list of currently most popular books"
    plt.show()
...
def testing_bookrecommend():
    ...
    assert list(actual[0].gca().lines[0].get_ydata()) == expected, \
        "should return a graph showing a list of recommended books and " \
        "each value for the books should be correct"
    expected = ['The Fifth Season', 'The Library at Mount Char',
                'The Kingdom of Copper', 'The Winter of the Witch',
                'The Lord of the rings', 'The Wonderful Wizard of Oz',
                'Magic for Liars', 'Red Sister']
    assert actual[1] == expected, "should return a list of the popular books"
    plt.show()
